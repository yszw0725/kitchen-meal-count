-- Phase 7: 標準パターン+例外の判定ロジックを共通関数に切り出す
--
-- get_day_board内に埋め込まれていた「標準パターン+meal_exceptions」の
-- 実喫食判定ロジックを effective_eats_status(date) として独立させ、
-- get_day_board (kitchen_overridesを最優先で加味するライブ表示用) と
-- get_monthly_summary (kitchen_overridesを含めない月次清算用, Phase7で追加)
-- の両方から再利用する。計算式を二重に持たないための土台。
--
-- 併せて get_day_board の各区分にcount_overrides適用有無のフラグ
-- (is_overridden) を追加する (S2で「修正済み」バッジ表示に使う)。
...
-- ---------------------------------------------------------------------------
-- get_day_board: effective_eats_status を土台にし、kitchen_overrides を
-- 最優先で上乗せする。count_overrides適用有無を is_overridden として追加。
-- ---------------------------------------------------------------------------
